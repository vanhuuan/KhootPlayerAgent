...
async def get_question(llm, context):
    try:
        ...
        # If no similar questions found, proceed with normal question processing
        ...
        if question:
            try:
                ...
                print(f"Error parsing question: {str(parse_error)}")
...
def get_llm_model(question_type=None):
    ...
    try:
        ...
        print(f"Error getting LLM model: {str(e)}")
...
async def get_answer(question: Question):
    try:
        ...
        traceback.print_exc()
        # Return a default answer to prevent the game from stopping
        default_answer = AnswerData(correct_options=["default_answer"], explanation="Error occurred")
...
async def enter_answer(question: Question, llm, context):
    try:
        ...
        print(f"Error entering answer: {str(e)}")
        traceback.print_exc()
        # Continue despite error
